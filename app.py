...
# TODO: connect to a local postgresql database -completed config.py
...
#  all model relationships and properties, as a database migration -completed
...
class PastShow(db.Model):
    ...
    def __repr__(self):
        return f'PastShow: id({self.id}), start_time({self.start_time})'
...
class Area(db.Model):
    ...
    def __repr__(self):
        return f'Area: city({self.city}), state({self.state})'
...
def venues():
    ...
    #       num_shows should be aggregated based on number of upcoming shows per venue -completed
    data = Area.query.all()
...
def search_venues():
    ...
    data = Venue.query.filter(func.lower(Venue.name).contains(func.lower(user_input))).all()
    response = {
        "count": len(data),
        "data": data
    }

    return render_template('pages/search_venues.html', results=response, search_term=user_input)
...
def shows():
    ...
    # TODO: replace with real venues data -completed
    #       num_shows should be aggregated based on number of upcoming shows per venue -completed
    # data = UpcomingShow.query.join(Venue).join(Artist).all()
    data = db.session.query(UpcomingShow, Venue, Artist).select_from(UpcomingShow).join(Venue).join(Artist).all()

    return render_template('pages/shows.html', result=data)
